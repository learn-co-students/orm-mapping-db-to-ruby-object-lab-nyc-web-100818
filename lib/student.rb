...
# DB[:conn].execute(sql) <- This will return an array of rows from the database that match our query.
...
#.map do |row|
  ...
  def self.students_below_12th_grade
    ...
    #this method below connects the DB and RB to execute the command of SQL and get the info as array
    DB[:conn].execute(sql).map do |row|
      self.new_from_db(row)
    end
  end
